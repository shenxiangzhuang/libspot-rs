...
impl fmt::Display for SpotError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "SPOT error (code {}): {}", self.code, self.message)
    }
}
...
#[link(name = "spot")]
extern "C" {
    fn spot_init(spot: *mut SpotRaw, q: c_double, low: c_int, discard_anomalies: c_int,
                level: c_double, max_excess: c_ulong) -> c_int;
...
impl SpotDetector {
    ...
    pub fn new(config: SpotConfig) -> SpotResult<Self> {
        ...
        unsafe {
            ...
            if status < 0 {
                return Err(Self::make_error(status));
            }
        }
    ...
    pub fn quantile(&self, q: f64) -> f64 {
        ...
        unsafe {
            spot_quantile(self.raw.as_ptr(), q)
        }
    }
    ...
    fn make_error(code: c_int) -> SpotError {
        let message = unsafe {
            let mut buffer = vec![0u8; 256];
            libspot_error(code, buffer.as_mut_ptr() as *mut c_char, buffer.len() as c_ulong);
            String::from_utf8_lossy(&buffer).trim_end_matches('\0').to_string()
        };
        
        SpotError { code, message }
    }
}
