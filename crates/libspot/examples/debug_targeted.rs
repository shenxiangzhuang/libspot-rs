//! Targeted debug test to compare Grimshaw estimator at divergence point
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Get the current tail data before processing this sample
    let tail_size = detector.tail_size();
    println!("Current tail size before step: {}", tail_size);
    ...
    // Export the current peaks data for manual comparison
    let mut file = File::create("/tmp/peaks_data_100k.txt")?;
    writeln!(file, "Tail size: {}", detector.tail_size())?;
    ...
    println!("Peaks data exported to /tmp/peaks_data_100k.txt");
